...
def get_forcast_per_component(series, n_steps_in, n_steps_out, train_test_size):
    ...
    # fit model
    model.fit(train_X, train_y, epochs=15, verbose=0)
...
data = pd.read_csv('new_data_backfill_forwfill.csv', index_col = 0,
                   header=0, parse_dates=True)

agg_by_cell = data.groupby(by = ['cell_num'])
print(len(agg_by_cell), 'celle')

n_steps_in, n_steps_out = 20, 6
...
with open('MAE_error_data_4_LSTM_with_STL_Decomposition_6.csv', 'w') as f:
    for key, value in dict2data.items():
        f.write('%s:%s\n' % (key, value))
with open('MAPE_error_data_4_LSTM_with_STL_Decomposition_6.csv', 'w') as f:
    for key, value in dict2MAPE.items():
        f.write('%s:%s\n' % (key, value))
with open('RMSE_error_data_4_LSTM_with_STL_Decomposition_6.csv', 'w') as f:
